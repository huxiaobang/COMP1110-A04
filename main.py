...
from planner import VALID_PREFERENCES, find_journeys, rank_journeys, LARGE_NETWORK_THRESHOLD
...
def display_menu():
    ...
    print("  8. Help")
    print("  9. Fetch real HK network from government APIs")
    print("  0. Exit")
    print("-" * 42)
...
def display_help():
  ...
  REAL DATA:
    Use [9] to fetch real MTR/bus data from HK government
    open-data APIs. Requires internet access.
...
def fetch_real_network_flow(network):
    """Fetch real HK transport data from government APIs and load it."""
    print("\n  Fetch Real HK Network from Government APIs")
    print("  This will download MTR station/fare data from data.gov.hk")
    print("  and combine it with curated bus routes and walking links.")
    confirm = read_input("  Continue? (y/n) [y]: ").lower()
    if confirm not in ("", "y", "yes"):
        print("  Cancelled.")
        return network

    try:
        from fetch_real_data import fetch_and_build_network
    except ImportError:
        print("  [!] fetch_real_data.py not found in the current directory.")
        return network

    print("  Fetching data from APIs... (this may take a few seconds)")
    try:
        new_network, output_path = fetch_and_build_network()
        print(
            f"  [OK] Real network loaded: {len(new_network.stops)} stops, "
            f"{len(new_network.segments)} segments."
        )
        if len(new_network.stops) > LARGE_NETWORK_THRESHOLD:
            print(f"  [i] Large network detected (>{LARGE_NETWORK_THRESHOLD} stops).")
            print("       Dijkstra algorithm will be used for journey search.")
        print(f"  [i] Network saved to: {output_path}")
        return new_network
    except Exception as exc:
        print(f"  [!] Failed to fetch real network: {exc}")
        print("       Keeping current network.")
        return network
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            network = fetch_real_network_flow(network)
        elif choice == "0":
            print("\n  Goodbye! Thank you for using Smart Public Transport Advisor.\n")
            break
        else:
            print(f"  [!] Invalid choice '{choice}'. Please enter 0-9.")
